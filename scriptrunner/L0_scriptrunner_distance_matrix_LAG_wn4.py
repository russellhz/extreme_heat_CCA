...
for LOC in LOCS:
    for PICTL in PICTLS:
        for LAG in LAGS:
            ...
            job_file = "jobs/distance_matrix_" + args_ + '_wn4.sbatch'
            open(job_file, 'a')

            with open(job_file, "w") as fh:
                fh.writelines("#!/bin/bash -l\n")
                fh.writelines("#PBS -q casper\n")
                fh.writelines("#PBS -N distance_matrix_LAG_wn4\n")
                fh.writelines("#PBS -A P04010022\n")
                fh.writelines("#PBS -l select=1:mem=30GB\n")
                fh.writelines("#PBS -l walltime=06:00:00\n")
                fh.writelines("#PBS -o out/distance_matrix_wn4.out\n")
                
                fh.writelines("module load ncarenv\n")
                fh.writelines("module load python\n")
                fh.writelines("ncar_pylib my_npl_clone_casper\n")
                fh.writelines("python ../py/L0/distance_matrix_LAG_wn4.py "  + args + " > logs/distance_matrix_LAG_wn4" + args_ +".log\n")
