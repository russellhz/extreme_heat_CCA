...
for LOC in LOCS:
    job_file = "jobs/" + 'tas_anom_AMJJAS_creator_'+ LOC +'.sbatch'
    open(job_file, 'a')
    ...
    with open(job_file, "w") as fh:
        ...
        fh.writelines("#SBATCH --output=out/tas_anom_AMJJAS.out.%j\n")
        fh.writelines("module load ncarenv\n")
        fh.writelines("module load python\n")
        fh.writelines("ncar_pylib my_npl_clone_casper\n")
        fh.writelines("python ../py/L0/tas_anom_AMJJAS_creator.py " + LOC + " > logs/tas_anom_AMJJAS_creator_" + LOC+ ".log\n")
