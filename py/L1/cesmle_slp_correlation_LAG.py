...
slp_data_dict = {}
for LOC in LOCS:
    fn = DIR + 'gridded_slp_z500/gridded_slp_z500_' + PICTL_TYPE + '_' + LOC + '_' + str(LAG) +  '.nc'
    
    # Find heatwave years
    dates = np.load(DIR + 'dates/top_dyn_composite_dates_' + PICTL_TYPE + '_' + LOC + '_' + str(LAG) + '.npy', allow_pickle=True)
...
df_all.to_csv(DIR+'cesmle_slp_correlation_lag' + str(LAG) + '.csv')
...
for fn in cesmle_fns:
    ...
    for LOC in LOCS:
        ...
        df_all = pd.concat([df_all, df_loc])
    print("Done with " + code)
df_all.to_csv(DIR+'cesmle_mean_slp_correlation_lag' + str(LAG) + '.csv')