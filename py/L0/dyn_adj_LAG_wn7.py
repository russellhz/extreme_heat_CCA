...
##################### INPUTS ###########################
# PICTL Files
tas_pictl_fn = PICTL_DIR + "TREFHT_" + PICTL + "_" + LOC + "_AMJJAS_anom_wn7.nc"
slp_pictl_fn = PICTL_DIR + "PSL_" + PICTL + "_" + LOC + "_AMJJAS_anom_wn7.nc"
...
for i in range(MJJA_days):
    similarity[i] = np.load(DIR + 'distance_matrices/distance_matrix_' + PICTL + '_' +  LOC + '_LAG' + str(LAG) + '_wn7_' + str(i) + '.npy')
...
ofn = DYN_ADJ_DIR + 'dyn_adj_' + fn_adder + '_wn7.nc'
...
rmse_xr.to_netcdf(RMSE_DIR + 'rmse_' + fn_adder + '_wn7.nc')
